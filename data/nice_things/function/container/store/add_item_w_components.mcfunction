...
$execute \
    if data entity @n[type=item_display,tag=nice_things.container.display,distance=..0.6] data.stored_item.components \
    unless data entity @n[type=item_display,tag=nice_things.container.display,distance=..0.6] data.stored_item{components:$(components)} \
        run return fail
